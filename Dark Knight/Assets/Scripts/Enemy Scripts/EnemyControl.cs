        ...
        if(curState == EnemyState.RUN || curState == EnemyState.PAUSE) {
            if(curState != EnemyState.ATTACK) {
                ...
                if(Vector3.Distance(transform.position, targetPosition) >= 2.1f) {
                    ...
                    //if running or pause state, and not attacking state, run to player
                }
            ...
            //set destination to current destination. dont want to move when enemy is attacking player
